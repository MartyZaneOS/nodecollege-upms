...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$confirm({
          ...
          onOk () {
            return new Promise((resolve, reject) => {
              operateApi.delUserOrg({id: record.id}).then((res) => {
                if (res.success) {
        ...
        this.addOrUpdateModal.form.validateFields((err, values) => {
          if (!err) {
            ...
            if (this.addOrUpdateModal.title === '添加机构') {
              ...
              operateApi.editUserOrg({...values, id: this.addOrUpdateModal.data.id}).then(res => {
                this.$message.info(res.message)
                this.init()
                this.addOrUpdateModal.loading = false
                this.addOrUpdateModal.visible = false
      ...
      selectRoleList () {
        operateApi.getUserRoleListByOrg({...this.role.pageObj, data: {orgCode: this.rowSelectData.key}}).then(res => {
          if (res.success) {
      ...
      selectUserList () {
        operateApi.getUserListByOrg({...this.user.pageObj, data: {orgCode: this.rowSelectData.key}}).then(res => {
          if (res.success) {
