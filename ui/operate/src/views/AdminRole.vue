<template>
  <a-row style="padding: 10px;">
        ...
        <template slot="operation" slot-scope="text, record, index">
          <div>
            <span>
              ...
              <a @click="() => showOrgAdminModal(record)" v-if="record.roleType === 1">授权管理员</a>
    ...
    <a-modal v-model="bindOrgAdminModal.visible" title="绑定管理员" width="1200px">
      <a-row>
        <a-col span="8">
          <a-tree
              :replace-fields="{children:'children', title:'title', key: 'key'}"
              :tree-data="bindOrgAdminModal.bindTreeData"
              :expanded-keys="bindOrgAdminModal.bindTreeExpandedKeys"
              v-model="bindOrgAdminModal.checkedKeys"
              @expand="(expandedKeys)=>{this.bindOrgAdminModal.bindTreeExpandedKeys = expandedKeys}"
              @select="showAdmin"
          />
        </a-col>
        <a-col span="16">
          <a-table :dataSource="bindOrgAdminModal.data" :columns="bindOrgAdminModal.columns" :pagination="bindOrgAdminModal.pagination"
                   size="small" bordered :loading="bindOrgAdminModal.loading"
                   :rowKey="record => (record.adminId)">
            <template slot="operation" slot-scope="text, record, index">
              <a @click="() => addAdminOrgRole(record)" v-if="!record.isBind">授权</a>
              <a @click="() => delAdminOrgRole(record)" v-if="record.isBind">解除授权</a>
            </template>
          </a-table>
          <NCPagination :page="bindOrgAdminModal.pageObj" @changePage="(page)=>{this.bindOrgAdminModal.pageObj = page}" @searchList="searchList"/>
        </a-col>
      </a-row>
      <template slot="footer">
        <a-button key="submit" type="primary" :loading="bindOrgAdminModal.loading" @click="bindOrgOk">
          保存/更新
        </a-button>
        <a-button key="back" @click="bindOrgAdminModal.visible = false">取消</a-button>
      </template>
    </a-modal>
  </a-row>
</template>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        bindOrgAdminModal: {
          ...
          rowSelection: {
            selectedRowKeys: [],
            onChange: (selectedRowKeys, selectedRows) => {
              this.bindOrgAdminModal.rowSelection.selectedRowKeys = selectedRowKeys
            }
          },
          columns: [
    ...
    methods: {
      ...
      searchList () {
        this.loading = true
        operateApi.getRoleList({...this.pageObj, data: {...this.search}}).then((res) => {
          if (res.success) {
            this.data = res.rows
      ...
      addModal () {
        ...
        if (this.addOrUpdateModal.productTree.length === 0) {
          operateApi.getProductList({data: {productUsage: 0}}).then((res) => {
            if (res.success) {
        ...
        this.$confirm({
          ...
          onOk () {
            return new Promise((resolve, reject) => {
              operateApi.delRole({id: record.id}).then((res) => {
                if (res.success) {
        ...
        this.addOrUpdateModal.form.validateFields((err, values) => {
          if (!err) {
            if (this.addOrUpdateModal.title === '添加角色') {
              operateApi.addRole({...values}).then(res => {
                if (res.success) {
              ...
              operateApi.editRole({...values, id: this.addOrUpdateModal.data.id}).then(res => {
                if (res.success) {
                  this.$message.info('成功！')
                  this.searchList()
                }
      ...
      showMenu (record) {
        ...
        operateApi.getProductMenuTree({productCode: record.productCode}).then(res => {
          ...
          operateApi.getRoleMenuList({pageSize: -1, data: {roleCode: record.roleCode}}).then(res => {
      ...
      bindOk () {
        operateApi.bindRoleMenu({sourceCodes: [this.bindModal.roleData.roleCode], targetCodes: this.bindModal.checkedKeys, navPlatform: this.bindModal.navSelect}).then(res => {
          this.searchList()
      ...
      showOrg (record) {
        ...
        operateApi.getOrgTree({data: {}}).then(res => {
          ...
          operateApi.getOrgTreeByRole({pageSize: -1, data: {roleCode: record.roleCode}}).then(res2 => {
      ...
      bindOrgOk () {
        operateApi.bindRoleOrg({sourceCodes: [this.bindOrgModal.roleData.roleCode], targetCodes: this.bindOrgModal.checkedKeys.checked}).then(res => {
          this.searchList()
          this.bindOrgModal.visible = false
        })
      },
      // 显示组织机构用户信息
      showOrgAdminModal (record) {
        this.bindOrgAdminModal.roleData = record
        this.bindOrgAdminModal.bindTreeData = []
        this.bindOrgAdminModal.bindTreeExpandedKeys = []
        this.bindOrgAdminModal.checkedKeys = []
        operateApi.getOrgTreeByRole({pageSize: -1, data: {roleCode: record.roleCode}}).then(res => {
          this.bindOrgAdminModal.bindTreeData = res.rows
          this.handleBindOrgTree(res.rows, this.bindOrgAdminModal.bindTreeExpandedKeys)
          this.bindOrgAdminModal.visible = true
        })
      },
      showAdmin (selectedKeys, info) {
        this.bindOrgAdminModal.orgCode = selectedKeys[0]
        if (selectedKeys.length > 0) {
          this.bindOrgAdminModal.loading = true
          operateApi.getAdminListByOrg({
            ...this.bindOrgAdminModal.pageObj.pageNum,
            data: {orgCode: selectedKeys[0]}
          }).then((res) => {
            if (res.success) {
              operateApi.getAdminListByRoleOrg({roleCode: this.bindOrgAdminModal.roleData.roleCode, orgCode: selectedKeys[0]}).then((res2) => {
                if (res2.success) {
                  ...
                  this.bindOrgAdminModal.data = res.rows
                  this.bindOrgAdminModal.pageObj.total = res.total
                }
                this.bindOrgAdminModal.loading = false
              })
            }
          })
        }
      ...
      // 删除菜单
      addAdminOrgRole (record) {
        let that = this
        this.$confirm({
          title: '绑定管理员机构角色！?',
          onOk () {
            return new Promise((resolve, reject) => {
              operateApi.addAdminOrgRole({
                adminId: record.adminId,
                roleCode: that.bindOrgAdminModal.roleData.roleCode,
                orgCode: that.bindOrgAdminModal.orgCode
              }).then((res) => {
                if (res.success) {
                  that.$message.info(res.message)
                  that.showAdmin([that.bindOrgAdminModal.orgCode])
                  resolve(res.message)
                } else {
                  reject(res.message)
                }
      ...
      // 删除菜单
      delAdminOrgRole (record) {
        let that = this
        this.$confirm({
          title: '解绑管理员机构角色！?',
          onOk () {
            return new Promise((resolve, reject) => {
              operateApi.delAdminOrgRole({
                adminId: record.adminId,
                roleCode: that.bindOrgAdminModal.roleData.roleCode,
                orgCode: that.bindOrgAdminModal.orgCode
              }).then((res) => {
