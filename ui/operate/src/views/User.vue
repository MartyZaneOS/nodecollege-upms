  ...
  <a-row style="padding: 10px;">
    ...
    <a-col span="24">
      <a-table :dataSource="userList" :columns="columns" :pagination="pagination" :loading="loading" size="small"
               bordered :rowKey="record => record.userId">
  ...
  export default {
    ...
    methods: {
        ...
        this.$confirm({
          ...
          onOk () {
            return new Promise((resolve, reject) => {
              console.log('删除')
              operateApi.delUser({userId: record.userId}).then((res) => {
                if (res.success) {
      ...
      resetPwd (record) {
        const that = this
        this.$confirm({
          title: '重置用户密码？重置后密码为系统配置管理中默认密码设置的值！',
          onOk () {
            return new Promise((resolve, reject) => {
              operateApi.resetUserPwd({userId: record.userId}).then((res) => {
                if (res.success) {
                  that.$message.info(res.message)
                  that.searchList()
                  resolve(res.message)
      ...
      showOrg (record) {
        ...
        operateApi.getUserOrgTree({data: {}}).then(res => {
          ...
          operateApi.getOrgTreeByUser({pageSize: -1, data: {userId: record.userId}}).then(res2 => {
      ...
      bindOrgOk () {
        operateApi.bindUserOrg({sourceIds: [this.bindOrgModal.adminData.userId], targetCodes: this.bindOrgModal.checkedKeys.checked}).then(res => {
          this.searchList()
      ...
      showRole (record) {
        ...
        operateApi.getRoleListByUser({data: {userId: record.userId}}).then(res => {
      ...
      getAdminPower () {
        ...
        operateApi.getUserPower({...this.powerModal.search}).then(res => {
