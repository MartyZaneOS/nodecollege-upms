    ...
    <a-col span="12">
      <a-card title="角色信息" size="small" :headStyle="{lineHeight: '40px'}">
        <a-table :dataSource="role.data" :columns="role.columns" :pagination="pagination" :loading="role.loading"
                 size="small" bordered :rowKey="record => record.roleCode + '-' + record.roleSource">
        </a-table>
        <NCPagination :page="role.pageObj" @changePage="(page)=>{this.role.pageObj = page}" @searchList="selectRoleList"/>
      </a-card>
    </a-col>
    <a-col span="12">
      <a-card title="用户信息" size="small" :headStyle="{lineHeight: '40px'}">
        <a-table :dataSource="user.data" :columns="user.columns" :pagination="pagination" :loading="user.loading"
                 size="small" bordered :rowKey="record => record.userId">
        </a-table>
        <NCPagination :page="user.pageObj" @changePage="(page)=>{this.user.pageObj = page}" @searchList="selectUserList"/>
      </a-card>
    </a-col>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        this.$confirm({
          ...
          onOk () {
            return new Promise((resolve, reject) => {
              tenantApi.delOrg({id: record.id}).then((res) => {
                if (res.success) {
        ...
        this.addOrUpdateModal.form.validateFields((err, values) => {
          if (!err) {
            ...
            if (this.addOrUpdateModal.title === '添加机构') {
              ...
              tenantApi.editOrg({...values, id: this.addOrUpdateModal.data.id}).then(res => {
                this.$message.info(res.message)
                this.init()
                this.addOrUpdateModal.loading = false
                this.addOrUpdateModal.visible = false
      ...
      selectRoleList () {
        tenantApi.getRoleListByOrg({...this.role.pageObj, data: {orgCode: this.rowSelectData.key}}).then(res => {
          if (res.success) {
      ...
      selectUserList () {
        tenantApi.getMemberListByOrg({...this.user.pageObj, data: {orgCode: this.rowSelectData.key}}).then(res => {
          if (res.success) {
            this.user.data = res.rows
            this.user.pageObj.total = res.total
          }
        })
      }
    }
